...
<head>
    ...
    <title>Cadastro de Obras - Biblioteca</title>
...
<style>
    * {
    ...
    /* ADICIONADO: select e input[type="number"] para estilização */
    input[type="text"],
    input[type="date"],
    input[type="tel"],
    ...
    <div class="container">
        ...
        <form method="post" enctype="multipart/form-data">
            <div class="form-title">
                ...
                <h2>Ficha da Obra</h2>
                <p>Preencha os dados da nova obra literária</p>
            </div>

            <div>
                <span>Título (Nome do Livro)</span>
                <input type="text" name="tituloLivro" placeholder="Digite o título da obra">
            </div>
            <div>
                <span>Ano de Publicação</span>
                <input type="number" name="anoPubli" placeholder="EX: 2025">
            </div>
            <div>
                <span>Categoria</span>
                <input type="text" name="categoriaLivro" placeholder="Romance, Ficção, Técnico...">
            </div>
            <div>
                <span>Editora</span>
                <?php
                echo "<select name='ideditora' id='nome'>";
                echo "<option value='' disabled selected>Selecione uma editora...</option>";
                
                $con_editora = new mysqli($svname, $user, $password, $db);
                if (!$con_editora) {
                    die("Falha na Conexão.");
                };

                $sql = "SELECT * from editoracadastro";
                $resulta = mysqli_query($con_editora, $sql);

                while ($linha = mysqli_fetch_array($resulta)) {
                    echo "<option value='" . $linha['ideditora'] . "'>" . $linha['nome'] . "</option>";
                }

                mysqli_close($con_editora);
                echo "</select>";
                ?>
            </div>
            <div>
                ...
                <?php
                echo "<select name='idautorCadastro' id='nomeAutor'>";
                echo "<option value='' disabled selected>Selecione um autor...</option>";
                
                $con_autor = new mysqli($svname, $user, $password, $db);
                if (!$con_autor) {
                    die("Falha na Conexão.");
                };

                $sql = "SELECT * from autorcadastro";
                $resulta = mysqli_query($con_autor, $sql);

                while ($linha = mysqli_fetch_array($resulta)) {
                    echo "<option value='" . $linha['idautorCadastro'] . "'>" . $linha['nomeAutor'] . "</option>";
                }

                mysqli_close($con_autor);
                echo "</select>";
                ?>
            </div>
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $publicacao = $_POST['anoPubli'];
    $categoria = $_POST['categoriaLivro'];
    $editora = $_POST['ideditora'];
    $autor = $_POST['idautorCadastro'];

    $sql = "INSERT INTO obracadastro(
    titulo,
    anoPublicacao,
    categoria,
    editoraCadastro_ideditora,
    autorCadastro_idautorCadastro
    ) VALUES
    ('$titulo', '$publicacao', '$categoria', '$editora', '$autor')";
